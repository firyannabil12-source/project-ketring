            ...
            <div class="filter-tabs" id="categoryFilter">
                ...
                @foreach ($categories as $cat)
                    <button class="filter-tab"
                        onclick="filterCategory(this, '{{ $cat }}')">{{ ucwords(strtolower($cat)) }}</button>
                @endforeach
            </div>
        </div>
        ...
        <table id="stokTable">
            ...
            <tbody>
                @forelse($menus as $menu)
                    <tr data-category="{{ $menu->category }}" data-name="{{ strtolower($menu->name) }}"
                        id="row-{{ $menu->id }}">
                        <td>
                        ...
                        <td>
                            <div class="action-btns">
                                <a href="{{ route('admin.menu.edit', $menu) }}" class="btn-edit">Edit</a>
                                <form method="POST" action="{{ route('admin.menu.destroy', $menu) }}"
                                    onsubmit="return confirm('Hapus menu \"{{ $menu->name }}\"? Tindakan ini tidak bisa dibatalkan.')">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" style="text-align: center; padding: 3rem; color: #94a3b8;">Belum ada menu. <a
                                href="{{ route('admin.menu.create') }}" style="color: #E8572A;">Tambah sekarang</a></td>
                    </tr>
                @endforelse
            </tbody>
        </table>
