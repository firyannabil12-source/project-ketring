...
@section('content')
    ...
    <div class="orders-header">
        <div class="stats-mini">
            <div class="stat-mini-card">
                <div class="val" style="color: #f59e0b;" id="countPending">{{ $orders->where('status', 'pending')->count() }}
                </div>
                ...
                @forelse($orders as $order)
                    <tr data-status="{{ $order->status }}" id="order-row-{{ $order->id }}">
                        ...
                        <td>
                            ...
                            <div class="customer-phone">HP: {{ $order->customer_phone }}</div>
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="event-date">Tanggal:
                                {{ $order->event_date ? $order->event_date->format('d M Y') : '-' }}</div>
                            <div class="event-addr" title="{{ $order->event_address }}">Alamat:
                                {{ $order->event_address ?? '-' }}</div>
                            @if ($order->latitude && $order->longitude)
                                <a href="https://www.google.com/maps?q={{ $order->latitude }},{{ $order->longitude }}"
                                    target="_blank"
                                    style="font-size: 0.75rem; color: #E8572A; text-decoration: none; font-weight: 600; display: inline-block; margin-top: 4px;">Lihat
                                    Lokasi</a>
                            @endif
                        </td>
                        ...
                        <td>
                            ...
                            @if ($order->payment_status === 'unpaid')
                                <span
                                    style="display:inline-block; margin-bottom: 5px; padding: 2px 8px; background: #fee2e2; color: #dc2626; border-radius: 10px; font-size: 0.7rem; font-weight: 700;">Belum
                                    Lunas</span><br>
                                <button onclick="openConfirmPaymentModal({{ $order->id }})"
                                    style="background: #16a34a; color: white; border: none; padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; cursor: pointer; font-weight: 600;">
                                    Konfirmasi Bayar
                                </button>
                            @elseif($order->payment_status === 'paid')
                                <span
                                    style="display:inline-block; margin-bottom: 5px; padding: 2px 8px; background: #dcfce7; color: #15803d; border-radius: 10px; font-size: 0.7rem; font-weight: 700;">Lunas</span><br>
                            ...
                            @elseif($order->payment_status === 'expired')
                                <span
                                    style="display:inline-block; margin-bottom: 5px; padding: 2px 8px; background: #f1f5f9; color: #64748b; border-radius: 10px; font-size: 0.7rem; font-weight: 700;">Kedaluwarsa</span>
                            @endif
                        </td>
                        <td>
    ...
    <div id="paymentModal"
        style="display:none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 2rem; border-radius: 16px; z-index: 9999; box-shadow: 0 10px 40px rgba(0,0,0,0.2); width: 90%; max-width: 400px;">
        <h3 style="margin-top:0; font-family:'Outfit',sans-serif; color:#0f172a;">Konfirmasi Pembayaran</h3>
        <p style="font-size:0.875rem; color:#64748b; margin-bottom:1.5rem;">Pesanan akan ditandai Lunas dan status berubah
            menjadi Diproses.</p>
        ...
        <form id="confirmPaymentForm">
            ...
            <div style="margin-bottom: 1rem;">
                <label
                    style="display:block; font-size:0.78rem; font-weight:700; margin-bottom:0.5rem; color:#475569;">Estimasi
                    Waktu Pembuatan (Opsional)</label>
                <input type="text" id="estimationTime" placeholder="Contoh: 45 Menit, 1 Jam"
                    style="width: 100%; padding: 0.7rem; border: 1.5px solid #e2e8f0; border-radius: 8px; box-sizing: border-box; font-family:'Inter',sans-serif; font-size:0.875rem;">
            </div>
            <div style="display: flex; gap: 0.5rem; justify-content: flex-end; margin-top: 1.5rem;">
                <button type="button" onclick="closeConfirmPaymentModal()"
                    style="padding: 0.6rem 1rem; border: none; background: #f1f5f9; color: #475569; border-radius: 8px; cursor: pointer; font-weight: 600;">Batal</button>
                <button type="submit"
                    style="padding: 0.6rem 1rem; border: none; background: #16a34a; color: white; border-radius: 8px; cursor: pointer; font-weight: 700;">Konfirmasi
                    & Simpan</button>
            </div>
        </form>
    </div>
            ...
            fetch(`/admin/pesanan/${orderId}/konfirmasi-pembayaran`, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        1000); // Reload to reflect full changes easily
                    } else {
